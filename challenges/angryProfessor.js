...
The first 3 students arrived on. The last 2 were late. The threshold is 3 students, so class will go on. Return YES. (THIS SHOULD BE NO)
...
for (let i = 0; i < a.length; i++) {
    a[i] > 0 ? positiveCount++ : negativeCount++
    if (negativeCount >= k) return 'NO'
    if (positiveCount > a.length - k) return 'YES' //I think this one would be better O than outside the loop if you don't have to run the whole list, although 1000 is short
}
// return 'YES'
